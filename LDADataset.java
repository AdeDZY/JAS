...
public class LDADataset implements Serializable{
	...
	public LDADataset(int M, int I, Dictionary globalDict){
		localDict = new Dictionary();	
		this.M = M;
		this.V = 0;
		this.I = I;
	...
	public void setDoc(String str, int idx){
		if (0 <= idx && idx < M){
			...
			int item = Integer.parseInt(words[0]);
			...
			Document doc = new Document(ids, item, str);
			docs[idx] = doc;
	...
	public static LDADataset readDataSet(BufferedReader reader){
		try {
			...
			LDADataset data = new LDADataset(M,I);
			for (int i = 0; i < M; ++i){
				line = reader.readLine();
				////System.out.println(line);
				data.setDoc(line, i);
	...
	public static LDADataset readDataSet(BufferedReader reader, Dictionary dict){
		try {
			...
			LDADataset data = new LDADataset(M, I, dict);
			for (int i = 0; i < M; ++i){
				line = reader.readLine();
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet(String [] strs, int I){
		LDADataset data = new LDADataset(strs.length, I);
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet(String [] strs, Dictionary dict, int I){
		//System.out.println("readDataset...");
		LDADataset data = new LDADataset(strs.length, I, dict);
